...
public class EvaluvationController {
	@Autowired
	private IEvaluationService ievaluvationService;

	// add evaluation
	@PostMapping("evaluation")
	private ResponseEntity<Evaluation> addEvaluation(@RequestBody Evaluation evaluation) {
		return new ResponseEntity<Evaluation>(ievaluvationService.addEvaluation(evaluation), HttpStatus.OK);
	}

	// get all evaluation
	@GetMapping("evaluation")
	private ResponseEntity<List<Evaluation>> listAllEvaluation() {
		return new ResponseEntity<List<Evaluation>>(ievaluvationService.listAllEvaluvation(), HttpStatus.OK);
	}
